...
INT_PTR CALLBACK CPUProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_DEBUG_SOURCEFILE:
			if(wmEvent == CBN_SELCHANGE) {
				DebugExt::SourceFileSelChanged();
			}
			return TRUE;

		case IDC_DEBUG_WATCH_LIST:
			if(wmEvent == LBN_SELCHANGE) {
				DebugExt::WatchListSelChanged();
			}
			return TRUE;
		...
		case IDC_DEBUG_SOURCECODE_LIST:
			if (wmEvent == LBN_DBLCLK)
			{
				bp = (struct tBreakpoint *)DialogBoxParam(hInst, (LPCTSTR)IDD_BREAKPOINT, hwndDlg, BreakpointProc, (LPARAM)0xFFFFFFFE);
				// if user cancels, nothing was added
				if (bp == NULL)
					break;
				// add it to the breakpoint listbox
